...
class ApprovalLemburController extends Controller
{
    ...
    {			
		...
		if(strtolower(Auth::user()->divisi->name) == 'administrator' &&  strtolower(Auth::user()->posisi->name) == 'administrator'){
			...
			$query = Lembur::with('user')->where('approve_status_by_leader', 'setuju')->orderBy('id','DESC');
        }else{
			$query = Lembur::with('user')->whereHas('user', function ($query) {
        ...
        return view('approval_form_lembur.index', ['absents' => $absent , 'total' => $total, 'divions' => $divions , 'positions' => $positions]);
    ...
    public function show($id)
    {
		...
		if($data){
			return view('approval_form_lembur.detail', ['data' => $data]);
		}else{

...
<div class="col-md-12 panel">
	...
	<h4>Edit Form Tidak Masuk No {{ 'CT-'.sprintf("%010d", $data->id) }}</h4>
	</div>
	<div class="col-md-12 panel-body" style="padding-bottom:30px;">
		<div class="col-md-12">
			<form class="cmxform" id="signupForm" method="post" enctype="multipart/form-data" action="/form-tidak-masuk/{{ Crypt::encrypt($data->id) }}" autocomplete="off">
			...
			@if(strtolower(Auth::user()->divisi->name) == 'administrator' &&  strtolower(Auth::user()->posisi->name) == 'administrator')
				...
				<div class="form-group form-animate-text @if($errors->has('nama_karyawan')) form-animate-error @endif" style="margin-top:40px !important;">
					...
					<option value=""> </option>
					@foreach ($users as $d)
						<option value="{{$d->id.'_'.$d->paid_leave}}" @if($data->id_user.'_'.$data->remaining_days_off == $d->id.'_'.$d->paid_leave) selected="selected" @endif>{{$d->name}}</option>
					@endforeach
				...
				<div class="form-group form-animate-text @if($errors->has('tanggal_mulai')) form-animate-error @endif" style="margin-top:40px !important;">
					<input type="text" class="form-text tanggal_mulai @if($errors->has('tanggal_mulai')) error @endif" id="tanggal_mulai" name="tanggal_mulai" required="" aria-required="true" value="{{ $data->date_start }}">
					<span class="bar"></span>
				...
				<div class="form-group form-animate-text @if($errors->has('tanggal_selesai')) form-animate-error @endif" style="margin-top:40px !important;">
					<input type="text" class="form-text tanggal_selesai @if($errors->has('tanggal_selesai')) error @endif" id="tanggal_selesai" name="tanggal_selesai" required="" aria-required="true" value="{{ $data->date_finish }}">
					<span class="bar"></span>
				...
				<div class="form-group form-animate-text @if($errors->has('kategori_ijin')) form-animate-error @endif" style="margin-top:40px !important;">
					...
					<select class="form-text @if($errors->has('kategori_ijin')) error @endif" id="kategori_ijin" name="kategori_ijin" required="" aria-required="true">
						<option value="sakit" @if($data->reason == 'sakit') selected="selected" @endif>Sakit</option>
						<option value="ijin" @if($data->reason == 'ijin') selected="selected" @endif>Ijin</option>
						<option value="cuti" @if($data->reason == 'cuti') selected="selected" @endif>Cuti</option>
					</select>
					<span class="bar"></span>
				...
				<div class="form-group form-animate-text @if($errors->has('keterangan')) form-animate-error @endif" style="margin-top:40px !important;">
					<textarea class="form-text @if($errors->has('keterangan')) error @endif" id="keterangan" name="keterangan" required="" aria-required="true">{{ $data->description }}</textarea>
					<span class="bar"></span>
				...
				<div class="form-group form-animate-text @if($errors->has('potongan')) form-animate-error @endif" style="margin-top:40px !important;">
					...
					<select class="form-text @if($errors->has('potongan')) error @endif" id="potongan" name="potongan" required="" aria-required="true">
						<option value="gaji" @if( $data->piece == 'gaji') selected="selected" @endif>Potongan Gaji</option>
						<option value="cuti" @if( $data->piece == 'cuti') selected="selected" @endif>Potongan Cuti</option>
					</select>
					<span class="bar"></span>

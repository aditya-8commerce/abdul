...
<div class="col-md-12 padding-0">
        <div class="col-md-12">
                  <div class="panel">
                    <div class="panel-heading"><h3>Data Lembur</h3> <a href="/form-lembur/create"><input class="submit btn btn-primary" type="button" value="Tambah Data"></a></div>
                    <div class="panel-body">
                      <div class="responsive-table">
                      <div id="datatables-example_wrapper" class="dataTables_wrapper form-inline dt-bootstrap no-footer"><div class="row">
					  <div class="col-sm-12">
					  <form class="cmxform" method="get" action="/list-form-lembur" novalidate="novalidate">
					 ...
					 @endif
					  ...
					  <a href="/list-form-lembur"><input class="submit btn btn-danger" type="button" value="Reset"></a></label>
                      ...
                      <tr>
						...
						@if(strtolower(Auth::user()->divisi->name) == 'administrator' &&  strtolower(Auth::user()->posisi->name) == 'administrator')
                          <td><a href="/form-lembur/{{ Crypt::encrypt($absent->id) }}"><input class="submit btn btn-default" type="button" value="Lihat"></a> |  <a href="/form-lembur/{{ Crypt::encrypt($absent->id) }}/edit"><input class="submit btn btn-warning" type="button" value="Rubah"></a> | <a onclick="event.preventDefault();document.getElementById('delete-form-{{ $loop->iteration }}').submit();"><input class="submit btn btn-danger" type="button" value="Hapus"></a>  | <a href="/form-lembur/print/{{ Crypt::encrypt($absent->id) }}" target="_blank"><input class="submit btn btn-primary" type="button" value="Print"></a></td>
							<form id="delete-form-{{ $loop->iteration }}" method="POST" action="/form-lembur/{{ $absent->id }}">
								{{ csrf_field() }}
								{{ method_field('DELETE') }}
							</form>
						@else
						<td><a href="/form-lembur/{{ Crypt::encrypt($absent->id) }}"><input class="submit btn btn-default" type="button" value="Lihat"></a> | 
						<a href="/form-lembur/print/{{ Crypt::encrypt($absent->id) }}" target="_blank"><input class="submit btn btn-primary" type="button" value="Print"></a>
						@if(empty($absent->approve_status_by_leader)) | 
						<a href="/form-lembur/{{ Crypt::encrypt($absent->id) }}/edit"><input class="submit btn btn-warning" type="button" value="Rubah"></a>
						@endif
						</td>
						@endif

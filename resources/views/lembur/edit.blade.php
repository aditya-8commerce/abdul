...
<div class="col-md-12 panel">
	...
	<h4>Edit Form Lembur</h4>
	</div>
	<div class="col-md-12 panel-body" style="padding-bottom:30px;">
		<div class="col-md-12">
			<form class="cmxform" id="signupForm" method="post" enctype="multipart/form-data" action="/form-lembur/{{ Crypt::encrypt($data->id) }}" autocomplete="off">
			...
			@if(strtolower(Auth::user()->divisi->name) == 'administrator' &&  strtolower(Auth::user()->posisi->name) == 'administrator')
				...
				<div class="form-group form-animate-text @if($errors->has('nama_customer')) form-animate-error @endif" style="margin-top:40px !important;">
					...
					@foreach ($customers as $d)
						<option value="{{$d->name}}" @if($data->customer_name == $d->name) selected="selected" @endif>{{$d->name}}</option>
					@endforeach
